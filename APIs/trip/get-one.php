 ...
 * @Last Modified time: 2016-03-16 23:35:19
 */
...
if($trip_id > 0){
	...
	$query = "SELECT T.title, T.start_time, T.end_time, C.city_name FROM `trips` T INNER JOIN `cities` C ON T.city_id = C.id WHERE T.trip_id='$trip_id'";
	...
	if(isset($query_row)){
		...
		$users_array = array();
		$users_query = "SELECT U.id, U.name FROM `trip_users` TU INNER JOIN `users` U ON TU.user_id = U.id WHERE TU.trip_id='$trip_id' ORDER BY U.name ASC";

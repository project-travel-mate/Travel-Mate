<?php
	...
	if (isset($_GET['src_city']) && isset($_GET['dest_city']) && isset($_GET['date'])) {
		...
		$src_url = 'http://api.railwayapi.com/name_to_code/
			station/' . $source . '/apikey/' . $train_api_code . '/';

		$response = json_decode(curl_url_call($src_url), true);
		$src_code = $response['stations'][0]['code'];

		$dest_url = 'http://api.railwayapi.com/name_to_code/
			station/' . $destination . '/apikey/' . $train_api_code . '/';

		$response = json_decode(curl_url_call($dest_url), true);
		$dest_code = $response['stations'][0]['code'];

		$trains_url = 'http://api.railwayapi.com/between/
			source/' . $src_code . '/dest/' . $dest_code . '/date/' . $_GET['date'] . '/
			apikey/' . $train_api_code . '/';
